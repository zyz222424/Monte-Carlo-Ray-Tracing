...
bool Model::LoadMaterial(string path, map<string, Material> &material_table) {
    ...
    while (file >> type) {
        if (type == "newmtl") {
            if (active_mtl) {  //第一个newntl还没有材质被读取
                material_table.insert(make_pair(material_name, material));  //map中插入一个材质名和材质
                material = Material();  //定义一个新材质
            } else {
                active_mtl = true;
            }
            file >> material_name;
...
bool Model::Load(string obj_path) {
    ...
    if (!file.is_open()) {  //打开obj文件
        cerr << "Fail to open obj " + obj_path << endl;
        return false;
    }
    bool return_value = true;
    map<string, Material> material_table;  //储存所有的材质信息

    string type;
    Vec3f v(0, 0, 0);
    Vec3f vn(0, 0, 0);
    string material_name;   //材质名称
    Material material = Material();
    std::array<int, 3> vertex_idx = {0, 0, 0};
    std::array<int, 3> normal_idx = {0, 0, 0};

    while (file >> type) {  //一个一个字符串读取
        if (type == "mtllib") {   //如果读取的是mtllib
            string mtl_file_name;
            file >> mtl_file_name;  //后面的就是文件名，直接输入
            auto pos = obj_path.find_last_of('/');
            string mtl_path = obj_path.substr(0, pos + 1) + mtl_file_name;  //材质文件的位置
            if (!LoadMaterial(mtl_path, material_table)) {   //加载材质文件
                return_value = false;
            ...
            if (it != material_table.end()) {  
                material = it->second;   //it在map表中存在就将值附给material
            } else {
            ...
            m_vertices.emplace_back(v);  //加入顶点
        } else if (type == "vn") {
            file >> vn.x() >> vn.y() >> vn.z();
            m_normals.emplace_back(vn);   //加入顶点法线
        }
        //f vertex-index/texture-index/normal-index
        else if (type == "f") {   //如果是面类型
            int index = 0;
            while (true) {
                auto ch = file.get();
                if (ch == ' ') {
                ...
                if (index >= 3) {
                    ...
                    auto *tri = new Triangle(this, vertex_idx, normal_idx);  //将每个三角形加入到tri中
                    tri->m_material = material;   //将材质加入到tri中
                    m_triangles.emplace_back(tri);   //

                    vertex_idx[1] = vertex_idx[2];   //这种操作可以将一个四边形分成2个三角形
